...
console.log(totalDePeliculas());

// Ejercicio 2
// funcion que devuelve el nombre de la pelicula con mayor mayorPopularidad
// HINT:
// campo a analizar _popularity_
// iterar todas las peliculas guardando:
// 1. el index o posicion de la peliluca con mayor popularidad
// 2. el valor de mayor popularidad como tal

function nombrePeliculaPopular() {
  let posicionMP = -1;
  let mayorPopularidad = 0;
  peliculas.forEach((pelicula, index) => {
    if (pelicula.popularity > mayorPopularidad) {
      mayorPopularidad = pelicula.popularity;
      posicionMP = index;
    }
  });

  //   console.log('mayor popularidad',mayorPopularidad);
  //   console.log('posicion de mayor popularidad',posicionMP);
  //   console.log('titulo de la pelicula:',peliculas[posicionMP].title);
  return peliculas[posicionMP].title;
}

//nombrePeliculaPopular()
console.log("Película mas popular: ", nombrePeliculaPopular());
...
function nombrePeliculaMasGeneros() {
  ...
  // determinar numero de generos mayor
  for (let i = 0; i < peliculas.length; i++) {
    if (peliculas[i].genre_ids.length >= mayorGenero) {
      mayorGenero = peliculas[i].genre_ids.length;
    }
  }

  // extrae peliculas con genero mayor
  for (let i = 0; i < peliculas.length; i++) {
    if (peliculas[i].genre_ids.length === mayorGenero) {
      peliculasMG.push(peliculas[i].title);
    }
  }
...
console.log("Película(s) con mas generos: ", nombrePeliculaMasGeneros());
...
function generosDeLaPrimeraPelicula() {
  ...
  for (let i = 0; i < generosPelicula0.length; i++) {
    for (let j = 0; j < generos.length; j++) {
      if (generosPelicula0[i] === generos[j].id) {
